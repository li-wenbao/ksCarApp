<template>
	<view class="content" style="background: #FFFFFF;">
		...
		<popup-layer ref="popupRef" :direction="tabDir" @close="close" :isTransNav="isTransNav" :navHeight="navHeight"
		 :tabHeight="tabHeight">
	...
	export default {
		...
		created: function() {
			...
			for (let i = 0; i < this.menuList.length; i++) {
				arr.push({
					'isActive': false,
					'tabDir': this.menuList[i].direction,
					'type': this.menuList[i].type
				});
				// titleArr.push({
				// 	'title': this.menuList[i].title,
				// 	'key': this.menuList[i].key
		...
		methods: {
			...
			pageTap(obj, key) {
				this.$refs.slFilterView.pageTap(obj, key);
			},
			getSelectedIndex() {
				return this.selectedIndex;
			},
			changeTitle(index, reTitle) {
				this.titleList[index].title = reTitle;
			},
			showMenuClick(index) {
				this.selectedIndex = index;
				if (this.statusList[index].type == 'popup') {
					if (this.statusList[index].isActive == true) {
						this.$refs.popupRef.close();
						this.statusList[index].isActive = false
					} else {
						this.tabDir = this.statusList[index].tabDir;
						this.menuTabClick(index);
						this.$refs.popupRef.show()
					}
				} else if (this.statusList[index].type == 'page') {
					uni.navigateTo({
			...
			repageClick(obj) {
				this.menuList.detailList.push(obj);
				this.menuList[this.selectedIndex].defaultSelectedIndex = this.menuList.detailList.lenth - 1;
